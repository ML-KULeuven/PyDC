...
activity(tintin):t+1 ~ finite([0.1:walk,0.4:shop,0.5:clean]) <- weather(brussels):t+1 ~= rainy.
activity(tintin):t+1 ~ finite([0.6:walk,0.3:shop,0.1:clean]) <- weather(brussels):t+1 ~= sunny.
